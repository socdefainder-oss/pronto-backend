...
catalogRoutes.delete("/products/:id", auth, async (req: AuthedRequest, res) => {
  try {
    ...
    // Impede exclusão se o produto tem pedidos associados (FK constraint)
    const orderItemCount = await prisma.orderItem.count({ where: { productId: id } });
    if (orderItemCount > 0) {
      return res.status(400).json({
        error: "Este produto não pode ser excluído pois possui pedidos associados. Desative-o em vez de excluir."
      });
    }
